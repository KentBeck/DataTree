...
fn test_large_value_splitting() {
    ...
    tree.put(1101, &large_value).unwrap();

    // Verify we can read it back
    let retrieved = tree.get(1101).unwrap().unwrap();
...
fn test_consecutive_large_values() {
    ...
    tree.put(1201, &value1).unwrap();
    tree.put(1202, &value2).unwrap();
    tree.put(1203, &value3).unwrap();

    // Verify all values can be retrieved
    assert_eq!(tree.get(1201).unwrap().unwrap(), value1);
    assert_eq!(tree.get(1202).unwrap().unwrap(), value2);
    assert_eq!(tree.get(1203).unwrap().unwrap(), value3);
...
fn test_large_value_updates() {
    ...
    tree.put(1301, &initial_value).unwrap();

    // Update with a different value
    let updated_value = vec![2u8; 20];
    tree.put(1301, &updated_value).unwrap();

    // Verify the update
    assert_eq!(tree.get(1301).unwrap().unwrap(), updated_value);
}
...
fn test_large_value_deletion() {
    ...
    tree.put(1401, &value).unwrap();

    // Delete the value
    tree.delete(1401).unwrap();

    // Verify it's gone
    assert!(tree.get(1401).unwrap().is_none());
...
fn test_mixed_size_values() {
    ...
    tree.put(1501, b"value1").unwrap();
    tree.put(1502, &vec![1u8; 20]).unwrap();
    tree.put(1503, b"value2").unwrap();
    tree.put(1504, &vec![2u8; 20]).unwrap();

    // Verify all values can be retrieved
    assert_eq!(tree.get(1501).unwrap().unwrap(), b"value1");
    assert_eq!(tree.get(1502).unwrap().unwrap(), vec![1u8; 20]);
    assert_eq!(tree.get(1503).unwrap().unwrap(), b"value2");
    assert_eq!(tree.get(1504).unwrap().unwrap(), vec![2u8; 20]);
