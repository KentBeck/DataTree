...
fn test_crc_verification_on_updates() {
    ...
    // Insert initial data
    tree.put(502, b"value1").unwrap();
    tree.put(503, b"value2").unwrap();

    // Verify data can be read
    assert_eq!(tree.get(502).unwrap().unwrap(), b"value1");
    assert_eq!(tree.get(503).unwrap().unwrap(), b"value2");

    // Update a value with same length
    tree.put(502, b"value1").unwrap(); // No change, just verify CRC works
...
fn test_multiple_page_corruption_scenarios() {
    ...
    tree.put(601, &value1).unwrap();
    tree.put(602, &value2).unwrap();
    tree.put(603, &value3).unwrap();
    ...
    assert!(tree.get(601).is_err());

    // Since we corrupted the root page, all subsequent reads should fail
    assert!(tree.get(602).is_err());
    assert!(tree.get(603).is_err());
}
...
fn test_error_handling_with_corrupted_pages() {
    ...
    // Insert some data
    tree.put(701, b"value1").unwrap();
    tree.put(702, b"value2").unwrap();
    ...
    assert!(tree.get(701).is_err());
    assert!(tree.get(702).is_err());
    assert!(tree.put(703, b"value3").is_err());
    assert!(tree.delete(701).is_err());
}
...
fn test_branch_page_crc_verification_on_updates() {
    ...
    // Update some data
    tree.put(1010, b"new_value10").unwrap();
